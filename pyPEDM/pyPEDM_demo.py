...
def select_mineral():
    ...
    root.geometry("240x220")  # Increase window height to accommodate new options
    ...
    result = {
        ...
        'plots': tk.StringVar(value="yes")  # New Plots option
    }
    ...
    # Configure column weights to shift content left
    main_frame.columnconfigure(0, weight=0)
    main_frame.columnconfigure(1, weight=1)
    
    # Folder selection function - move Browse button after Select data folder label
    tk.Label(main_frame, text="Select data folder:", 
             font=("Arial", 10)).grid(row=0, column=0, pady=(3, 0), sticky="e", padx=(20, 5))
    ...
    folder_entry.grid(row=1, column=0, columnspan=2, pady=(0, 5), padx=(5, 5), sticky="ew")
    
    tk.Label(main_frame, text="Mineral:", 
             font=("Arial", 10)).grid(row=2, column=0, pady=(3, 2), sticky="w", padx=(35, 5))
    
    mineral_var = tk.StringVar(value="apatite")
    mineral_menu = ttk.Combobox(main_frame, textvariable=mineral_var, 
                               values=["apatite", "zircon"], 
                               state="readonly", font=("Arial", 10), width=8, justify="center")
    ...
    tk.Label(main_frame, text="Zeta:", 
             font=("Arial", 10)).grid(row=3, column=0, pady=(2, 2), sticky="w", padx=(35, 5))
    
    zeta_var = tk.StringVar(value="390")
    zeta_entry = ttk.Entry(main_frame, textvariable=zeta_var, font=("Arial", 10), width=8, justify="center")
    ...
    tk.Label(main_frame, text="Rho-d:", 
             font=("Arial", 10)).grid(row=4, column=0, pady=(2, 2), sticky="w", padx=(35, 5))
    
    rho_d_var = tk.StringVar(value="1.322E6")
    rho_d_entry = ttk.Entry(main_frame, textvariable=rho_d_var, font=("Arial", 10), width=8, justify="center")
    rho_d_entry.grid(row=4, column=1, pady=(2, 2), sticky="w", padx=(0, 20))
    
    # Plots option - newly added
    tk.Label(main_frame, text="Plots:", 
             font=("Arial", 10)).grid(row=5, column=0, pady=(2, 2), sticky="w", padx=(35, 5))
    
    plots_var = tk.StringVar(value="yes")
    plots_menu = ttk.Combobox(main_frame, textvariable=plots_var, 
                             values=["yes", "no"], 
                             state="readonly", font=("Arial", 10), width=8, justify="center")
    plots_menu.grid(row=5, column=1, pady=(2, 2), sticky="w", padx=(0, 20))
    
    # Status label - reduce spacing
    status_label = tk.Label(main_frame, text="", font=("Arial", 10), fg="red")
    status_label.grid(row=6, column=0, columnspan=2, pady=(2, 2), padx=(5, 5))
    ...
    def confirm_selection():
        ...
        if validate_parameters():
            ...
            result['plots'].set(plots_var.get())
             
            # Exit main loop to continue program execution, but keep window open
            root.quit()  # Restore main loop exit
            root.destroy()  # Keep commented, do not destroy window
    ...
    button_frame.grid(row=7, column=0, columnspan=2, pady=(0, 20), padx=(20, 20), sticky="ew")
    ...
    return {
        ...
        'plots': result['plots'].get()  # Return Plots option
    }
...
plots = mineral_params['plots']  # Get Plots option
# print(f"\033[94m✓ Selected folder: {foldername1}\033[0m")
...
print(f"\033[94m✓ Plots option: {plots}\033[0m")  # Print Plots option
